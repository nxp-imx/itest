...
int v2x_cipher_aes_ccm_001(void){
    ...
    for (msg_size = 16; msg_size <= 128; msg_size+= 1) {
        ...
        ASSERT_EQUAL(icrypto_cipher_one_go(msg, expected_enc, msg_size, ICRYPTO_AES_128_CCM, aes_128_key_data, buff_encr + msg_size + 16, 12, NULL, 0, 16), (int)(msg_size + 16));
        ...
        cipher_args.output_size = msg_size + 16 + 12;
        ASSERT_EQUAL(hsm_cipher_one_go(sg0_cipher_hdl, &cipher_args), HSM_NO_ERROR);
        ASSERT_EQUAL(icrypto_cipher_one_go(msg, expected_enc, msg_size, ICRYPTO_AES_192_CCM, aes_192_key_data, buff_encr + msg_size + 16, 12, NULL, 0, 16), (int)(msg_size + 16));
        ASSERT_EQUAL(memcmp(buff_encr, expected_enc, msg_size + 16), 0);
        ...
        cipher_args.output_size = msg_size + 16 + 12;
        ASSERT_EQUAL(hsm_cipher_one_go(sg0_cipher_hdl, &cipher_args), HSM_NO_ERROR);
        ASSERT_EQUAL(icrypto_cipher_one_go(msg, expected_enc, msg_size, ICRYPTO_AES_256_CCM, aes_256_key_data, buff_encr + msg_size + 16, 12, NULL, 0, 16), (int)(msg_size + 16));
        ASSERT_EQUAL(memcmp(buff_encr, expected_enc, msg_size + 16), 0);
