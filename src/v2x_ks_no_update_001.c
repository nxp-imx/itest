...
int v2x_ks_no_update_001(void){
    ...
    ASSERT_EQUAL(save_test_ctx(&ctx, sizeof(test_ctx_t), "v2x_ks_max_update_0_test_ctx.bin"), 1);
...
int v2x_ks_no_update_001_part2(void){
    ...
    ASSERT_EQUAL(load_test_ctx(&ctx, sizeof(test_ctx_t), "v2x_ks_max_update_0_test_ctx.bin"), 1);
    ...
    gen_key_args.out_key = NULL;
    // GEN SM4 KEY + STORE IN NVM
    hsm_err_t ret = hsm_generate_key(sg0_key_mgmt_srv, &gen_key_args);
    printf("ret=%d\n", ret);
    ASSERT_NOT_EQUAL(ret, HSM_NO_ERROR);
